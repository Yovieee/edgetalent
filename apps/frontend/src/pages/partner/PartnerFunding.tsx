...
export default function PartnerFunding(): React.ReactElement {
  ...
  const filtered = fundingOpportunities.filter((opp) => {
    ...
    const matchQuery = opp.title.toLowerCase().includes(searchFundingQuery.trim().toLowerCase()) ||
      opp.description.toLowerCase().includes(searchFundingQuery.trim().toLowerCase());
